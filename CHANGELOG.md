...
### 改进
- **`group_reply_mode` 配置项**：WebUI 改为下拉选项（`options` + `labels`），避免手填错误值。
...
- **按周原始数据导出**：新增管理员指令 `/weekly raw`，导出全部文档按周统计 CSV（发布篇数、作者数量、字数）。
  - 优先尝试直接发送 CSV 文件到会话
  - 平台不支持文件发送时自动回退为本地落盘并返回路径

## [v0.29.0] - 2026-04-10

### 新增
- **群聊发送者识别**：利用 `event.get_sender_name()` 优化群聊体验
  - 系统提示词明确标注当前对话用户
  - 对话历史标注来源（[群友]/[NovaBot]）
  - 减少群聊中上下文混淆问题

- **Token 限额可配置**：`token_daily_limit` 配置项生效
  - 默认值 50000，可在管理面板调整

### 修复
- **群聊幻觉问题**：多方面优化减少 Agent 编造信息
  - 系统提示词新增"基于事实回答"原则
  - 搜索工具返回空结果时明确告知"不要编造答案"
  - 群聊对话历史从 5 轮减少到 1 轮
  - 搜索结果内容长度从 300 增加到 500 字符
...
### 新增
- **Agent 智能化升级**：多个指令改用 Agent 处理，提升智能化程度
  - `/collab find <主题>` - 使用 `SmartCollaborationTool` 智能推荐协作伙伴
  - `/partner [主题]` - 使用 Agent 智能推荐学习伙伴/导师
  - `/trajectory topic <主题>` - 使用 Agent 智能分析主题相关活动

- **功能介绍文档**：新增 `docs/NovaBot功能介绍.md`
  - 面向 NOVA 社团成员的宣传文档
  - 按场景/身份的使用指南
  - 完整功能一览
  - 快速开始指南
...
### 改进
- **协作推荐权重优化**：修复 `/collab find` 返回主题无关推荐的问题
  - 主题相关性权重提高（轨迹 0.4，文档 0.5）
  - 共同协作伙伴权重降低（0.1）
  - 增加主题匹配优先级排序
  - 记录具体文档标题作为推荐原因

- **指令回退机制**：Agent 调用失败时自动回退到基础规则匹配输出
...
- **成员轨迹数据未初始化**：同步后自动从文档元数据提取发布记录
- **`Storage.get_all_members` 不存在**：使用正确的 `load_members()` 方法
- **新增 `/sync collab` 命令**：同时初始化协作网络和成员轨迹
